...
User ID: ${user.id}`;
    ...
    return handleToolError(error, "spotify_get_user_profile");
  }
}
...
export async function getTopItems(args: TopItemsArgs): Promise<ToolResponse> {
  try {
    ...
    return handleToolError(error, "spotify_get_top_items");
  }
}
...
export async function getRecentlyPlayed(args: RecentlyPlayedArgs): Promise<ToolResponse> {
  try {
    ...
    return handleToolError(error, "spotify_get_recently_played");
  }
}
